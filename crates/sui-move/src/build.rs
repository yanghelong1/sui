...
pub fn resolve_published_at_manifest_field(package_root: PathBuf) -> anyhow::Result<()> {
    let manifest_string =
    ...
    let lookup: BTreeMap<String, String> = manifest
        ...
        .collect();
    let published_at = if let Some(addr) = lookup.get("published-at") {
        // FIXME!
        println!("published at {}", addr);
    } else {
        println!("NO published at");
    };
    Ok(published_at)
}
